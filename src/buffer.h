...
namespace Buffer {
	...
	std::vector<u8> read_bytes(const std::vector<u8>& bytes, size_t n, size_t& offset);

	void write_i8(std::vector<u8>& buffer,  i8 value);
	void write_i16(std::vector<u8>& buffer, i16 value);
	void write_i32(std::vector<u8>& buffer, i32 value);
	void write_i64(std::vector<u8>& buffer, i64 value);

	void write_u8(std::vector<u8>& buffer,  u8 value);
	void write_u16(std::vector<u8>& buffer, u16 value);
	void write_u32(std::vector<u8>& buffer, u32 value);
	void write_u64(std::vector<u8>& buffer, u64 value);

	void write_bytes(std::vector<u8>& buffer, const std::vector<u8>& bytes);
}
